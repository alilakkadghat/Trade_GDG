...
function ToggleGroup({ title, desc, on }: { title: string; desc: string; on: boolean }) {
    return (
        <div className="flex items-start justify-between gap-4">
            ...
            <button
                className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full transition-colors focus:outline-none ring-2 ring-transparent focus:ring-secondary focus:ring-offset-2 focus:ring-offset-surface-lowest ${on ? "bg-secondary" : "bg-surface-highest"
                    }`}
            >
                <span
                    className={`inline-block h-3.5 w-3.5 transform rounded-full bg-surface-lowest shadow ring-0 transition duration-200 ease-in-out ${on ? "translate-x-2" : "-translate-x-2"
                        }`}
                />
            </button>
        </div>

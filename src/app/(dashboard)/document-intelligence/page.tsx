...
export default function DocumentIntelligencePage() {
  ...
  >([]);
  const [complianceIssues, setComplianceIssues] = useState<{ type: string; message: string }[]>([]);
  ...
  const runExtraction = (text: string) => {
    ...
    const extractedHsCode = text.includes("999999")
      ? "999999"
      : text.includes("8471")
        ? "8471"
        : "None";

    const extractedProduct = text.includes("Laptops")
      ? "Laptops"
      : text.includes("Electronics")
        ? "Electronics"
        : "Unknown";

    const extractedDoc = text.toLowerCase().includes("invoice") ? "Invoice" : "Unknown Document";
  ...
  const handleFile = async (file: File) => {
    ...
    if (!file.type.includes("text") && !file.name.endsWith(".txt")) {
      setToastMessage("🚫 Unsupported file type. Please upload a .txt document.");
      setTimeout(() => setToastMessage(null), 3000);
      return;
    }
    ...
    <div className="space-y-6 max-w-[1200px] mx-auto pt-2 relative">
      ...
      <div className="border border-ghost rounded-xl p-12 bg-surface/30 flex flex-col items-center justify-center gap-6">
        <div className="text-center">
          <p className="text-muted-foreground font-medium mb-1">Select a document to analyze</p>
        ...
        {loadingStage ? (
          <div className="flex flex-col items-center gap-3 py-4">
            ...
            <p className="text-secondary font-medium tracking-wide animate-pulse">{loadingStage}</p>
          </div>
        ) : (
          <input
            type="file"
        ...
        <div className="space-y-6 mt-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          {errors.length === 0 ? (
            <div className="bg-green-500/10 border border-green-500/30 p-4 rounded text-green-600 dark:text-green-400">
              <h2 className="font-bold mb-2">✅ Document Validated</h2>
              <p className="text-sm">All extracted data matches expected values.</p>
            </div>
          ) : (
            <div className="bg-red-500/10 border border-red-500/30 p-4 rounded text-red-700 dark:text-red-400">
              <h2 className="font-bold mb-2">⚠️ Validation Errors</h2>
          ...
          {complianceIssues.length > 0 && (
            <div className="bg-yellow-500/10 border border-yellow-500/30 p-4 rounded text-yellow-700 dark:text-yellow-400">
              <h2 className="font-bold mb-2">⚖️ Compliance Checks</h2>
              <ul className="space-y-1 ml-2 list-disc list-inside">
              ...
              {validationResults.map((res, idx) => (
                ...
                >
                  <div className="flex justify-between">
                    <span className="text-xs font-semibold uppercase opacity-70">{res.field}</span>
                    <span
                  ...
                  <div className="text-sm">
                    ...
                    <div className="text-muted-foreground">Expected: {res.expected}</div>
                  </div>
                </div>
              ))}
            </div>
